...
public class Notifier {
    ...
    public static void sendEmail(Customer customer, Order finishedOrder){
        ...
        try {
            ...
            textBodyPart.setText(String.format("Dear %s, %n We are pleased to inform you that your %s  is now ready and available to be taken! %n You'll be pleased to know that your %s is priced at %.2f$,%n %s %n which we believe is a great value for the quality we offer. %n Thank you for choosing us. We hope to serve you again soon. %n Best regards, %n Carpet Cleaning Mart.%n", customer.getName(), finishedOrder.getCategory().toString().toLowerCase(), finishedOrder.getDescription(), finishedOrder.getPrice(), discount > 0 ? discountMsg : "")); // Set the message body text
